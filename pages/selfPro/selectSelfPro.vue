		...
		<view class="select-show-box">
			<scroll-view class="select-box-scroll" scroll-y>
				...
				<view class="show-pro-size">
					<view class="item-show-pro-size" v-for="(item, idx) in sizeArr" :key="idx">
						...
						<view class="show-pro-size-tags">
							<view :class="['item-size-tag', {'select-size-active': val.isSelect}]" v-for="(val, i) in item.childlist" :key="i" @tap="toSelectProSize(idx, i)">
								<span>{{val.size}}</span>
								<view v-if="val.isSelect" class="item-select-active-icon"><image src="/static/start-icon.png" mode=""></image></view>
							</view>
						</view>
					</view>
					...
					.show-pro-num {
						...
						.to-minus-num {
							...
							&::before {
								...
								width: 20upx;
								height: 4upx;
								background-color:$text-color-main-black;
							}
						}
						.to-show-num {
							width: 60upx;
